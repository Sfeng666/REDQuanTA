...
# Based on code/reference/sample_structure_comparison/code/scripts/plot_power_comparison.Rmd
...
summary_stats_str <- gsub(",", ".", summary_stats) # Filenames use dots
...
# Define Sample Structures manually (since input file might not exist)
sp_structure <- data.frame(
  n_strain = c(4, 6, 8, 10, 12, 14, 6, 9, 12, 15, 18, 21),
  n_replicate = c(3, 3, 3, 3, 3, 3, 2, 2, 2, 2, 2, 2)
)
sp_structure$total_samples <- sp_structure$n_strain * sp_structure$n_replicate * 2 # 2 populations
...
load_data <- function(chr_name) {
  ...
  for (i in 1:nrow(sp_structure)) {
    num_ind <- sp_structure$n_strain[i]
    num_rep <- sp_structure$n_replicate[i]
    
    # Construct struct_id: n2_i{ind}_r{rep}
    struct_id <- paste0("n2_i", num_ind, "_r", num_rep)
    
    # Path to result file
    # Expected: base_dir/{struct_id}/{chr_name}/tpr_fpr_matrix_{chr_name}.csv
    result_file <- file.path(base_dir, struct_id, chr_name, paste0("tpr_fpr_matrix_", chr_name, ".csv"))
    
    if (file.exists(result_file)) {
      cat("Reading:", result_file, "\n")
      data <- read.csv(result_file, header = TRUE, row.names = 1, check.names = FALSE)
      
      # Extract adaptive TPR (row "QST_0.8")
      adaptive_rows <- grep("QST", rownames(data), value = TRUE)
      
      if (length(adaptive_rows) > 0) {
        adaptive_row <- adaptive_rows[1] 
        tpr_values <- as.numeric(data[adaptive_row, ])
        ratios <- colnames(data)
        
        for (j in 1:length(tpr_values)) {
...
# Per-population version: x-axis = total_samples / 2
